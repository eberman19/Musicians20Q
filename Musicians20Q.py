...
musicians = []
RowCount = 0
with open('MusiciansFakeFile2.txt', newline='') as inputfile:
    for row in csv.reader(inputfile):
        RowCount = RowCount + 1
        musicians.append(Musician2(row[0], row[1], row[2], row[3], row[4], row[5],row[6], row[7], row[8],row[9], row[10], row[11]))
DS = DataSet2(musicians)
...
def YesCallBack():
    ...
    DS.update(answer)
    if DS.ZeroCount < 9:
        v.set("Does the musician/band have this charactertic: %s?" % (Musician2.Characteristic[DS.FrequentTopic]))
    if DS.ZeroCount == 9:
...
def NoCallBack():
    ...
    DS.update(answer)
    if DS.ZeroCount < 9:
        v.set("Does the musician/band have this characteristic: %s?" % (Musician2.Characteristic[DS.FrequentTopic]))
    if DS.ZeroCount == 9:
        DS.MusicianList.sort(key=lambda MusicianGuess: MusicianGuess.Frequency, reverse = True)
...
f = open('MusiciansFakeFile2.txt','w')
for x in range(len(musicians)):
